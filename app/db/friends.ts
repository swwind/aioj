...
export async function deleteOldFriend(myname: string, friendname: string): Promise<Result<void, string>> {
  ...
  const updres1 = await users.updateOne({ username: myname }, { $set: { friends: me.friends.filter((name) => name !== friendname) } });
  const updres2 = await users.updateOne({ username: friendname }, { $set: { friends: fd.friends.filter((name) => name !== myname) } });
  if (!updres1.result.ok || !updres2.result.ok) {
    return Result.error(SERVER_ERROR);
  }
